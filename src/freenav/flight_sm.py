...
class FlightFSM_Init(FlightFSM_Default):
    ...
    def new_position(self, fsm):
        ctxt = fsm.getOwner()
        if ctxt.is_initialised() and (ctxt.ground_speed > ctxt.TAKEOFF_SPEED) :
            fsm.getState().Exit(fsm)
            # No actions.
            pass
            fsm.setState(FlightFSM.Air)
            fsm.getState().Entry(fsm)
        elif ctxt.is_initialised() and (ctxt.ground_speed < ctxt.STOPPED_SPEED) :
            fsm.getState().Exit(fsm)
            # No actions.
            pass
